...
class ArgentinaMicDtaService extends BaseWebserviceService
{
    ...
    private function registrarTitEnvios(Voyage $voyage): array
    {
        try {
            ...
            // FLUJO 2 PASOS AFIP: RegistrarTitEnvios + RegistrarEnvios
            $allTracks = [];

            foreach ($voyage->shipments as $shipment) {
                // PASO 1: RegistrarTitEnvios (solo título)
                $transactionId1 = 'TIT_' . time() .$transactionId1 = substr(time(), -8) . str_pad($shipment->id, 2, '0', STR_PAD_LEFT); '_' . $shipment->id;
                $xmlTitulo = $this->xmlSerializer->createRegistrarTitEnviosXml($shipment, $transactionId1);
                
                $this->logOperation('info', 'XML Título enviado a AFIP', [
                    'xml_length' => strlen($xmlTitulo),
                    'transaction_id' => $transactionId1,
                    'shipment_id' => $shipment->id
                ]);

                // Enviar título a AFIP
                $responseTitulo = $soapClient->__doRequest(
                    $xmlTitulo,
                    $this->getWsdlUrl(),
                ...
                if ($responseTitulo === null || $responseTitulo === false) {
                    throw new Exception("Error en RegistrarTitEnvios para shipment {$shipment->id}");
                }

                // PASO 2: RegistrarEnvios (para generar TRACKs)
                $transactionId2 = substr(time(), -8) . str_pad($shipment->id, 2, '0', STR_PAD_LEFT);
                $xmlEnvios = $this->xmlSerializer->createRegistrarEnviosXml($shipment, $transactionId2);
                
                $this->logOperation('info', 'XML Envíos enviado a AFIP', [
                    'xml_length' => strlen($xmlEnvios),
                    'transaction_id' => $transactionId2,
                    'shipment_id' => $shipment->id
                ]);

                // Enviar envíos a AFIP (esto genera TRACKs)
                $responseEnvios = $soapClient->__doRequest(
                    $xmlEnvios,
                    $this->getWsdlUrl(),
                    'Ar.Gob.Afip.Dga.wgesregsintia2/RegistrarEnvios',
                    SOAP_1_1,
                    false
                );

                if ($responseEnvios === null || $responseEnvios === false) {
                    throw new Exception("Error en RegistrarEnvios para shipment {$shipment->id}");
                }

                // Procesar respuesta y extraer TRACKs del PASO 2
                $tracks = $this->extractTracksFromResponse($responseEnvios);
                if (empty($tracks)) {
