{{-- resources/views/admin/container-types/index.blade.php --}}
<x-app-layout>
    {{-- Header --}}
    <x-slot name="header">
        <div class="flex items-center justify-between gap-3">
            ...
            <a href="{{ route('admin.container-types.create') }}"
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm p-3 sm:p-4">
                <form method="GET" action="{{ route('admin.container-types.index') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    @foreach ($columns as $col)
                                        ...
                                        <td class="px-3 sm:px-4 py-2.5 text-gray-700 max-w-xs truncate" title="{{ is_scalar($value) ? (string) $value : '' }}">
                                            ...
                                            @else
                                                {{ is_scalar($value) ? Str::limit((string) $value, 50) : json_encode($value) }}
                                            @endif
                                        </td>
                                    @endforeach
                                    ...
                                    <td class="px-3 sm:px-4 py-2.5 text-right whitespace-nowrap">
                                        <div class="inline-flex gap-1">
                                            <a href="{{ route('admin.container-types.edit', $item) }}"
                                               class="px-2 py-1.5 rounded-md border border-gray-200 text-gray-700 hover:bg-gray-50"
                                               title="Editar">
                                                {{ __('Editar') }}
                                            </a>
                                            <form method="POST" action="{{ route('admin.container-types.destroy', $item) }}"
                                    ...
                                    <td colspan="{{ 2 + count($columns) }}" class="px-6 py-16 text-center">
                                        <div class="max-w-md mx-auto">
                                            ...
                                            <h3 class="text-lg font-semibold text-gray-800">
                                                {{ $q ? 'Sin resultados' : 'Aún no hay tipos de contenedor' }}
                                            </h3>
