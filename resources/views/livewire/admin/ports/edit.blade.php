...
<div class="max-w-4xl mx-auto" 
     ...
     "
     x-on:confirm-discard-changes.window="showDiscardModal = true"
     x-on:toast.window="beforeUnload = false">
    ...
    <form wire:submit.prevent="save" class="space-y-6">
                    ...
                    <div class="flex items-center space-x-4">
                        <div>
                            <h1 class="text-xl font-semibold text-gray-900">
                                Editar Puerto: {{ $port->name }}
                            </h1>
                            <p class="mt-1 text-sm text-gray-600">
                                Código: <span class="font-mono font-medium">{{ $port->code }}</span> • 
                                País: {{ $port->country->name ?? 'No asignado' }}
                            </p>
                        </div>
                        @if($hasUnsavedChanges)
                            <div class="flex items-center px-2 py-1 bg-yellow-100 text-yellow-800 rounded text-xs">
                    ...
                    <div class="flex items-center space-x-3">
                        @if($hasUnsavedChanges)
                            <button type="button" 
                                    ...
                                    class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 transition-colors"
                        ...
                        <button type="submit" 
                                class="px-4 py-2 text-sm font-medium text-white {{ $hasUnsavedChanges ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-indigo-600 hover:bg-indigo-700' }} border border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                            ...
                            <span wire:loading.remove wire:target="save">
                                {{ $hasUnsavedChanges ? 'Guardar Cambios' : 'Actualizar Puerto' }}
                            </span>
                ...
                <div class="flex items-center justify-between text-sm">
                    <div class="flex items-center space-x-6 text-gray-600">
                        ...
                        <span>
                            ...
                            {{ $portTypes[$port_type] ?? $port_type }}
                        </span>
                        <span>
                            <strong class="text-gray-900">Categoría:</strong>
                            {{ $portCategories[$port_category] ?? $port_category }}
                        </span>
                    </div>
                    <div class="text-xs text-gray-500">
                        Creado: {{ $port->created_at?->format('d/m/Y H:i') ?? 'No disponible' }} • 
                        Actualizado: {{ $port->updated_at?->format('d/m/Y H:i') ?? 'No disponible' }}
                    </div>
                </div>
            </div>
            ...
            <div class="border-b border-gray-200">
                <nav class="flex space-x-8 px-6" aria-label="Tabs">
                    <button type="button" 
                            ...
                            class="py-2 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'basic' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} transition-colors">
                        Información Básica
                        @if($hasUnsavedChanges && in_array($activeTab, ['basic']))
                            <span class="ml-1 w-2 h-2 bg-yellow-400 rounded-full inline-block"></span>
                        @endif
                    </button>
                    <button type="button" 
                            wire:click="setTab('location')"
                            class="py-2 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'location' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} transition-colors">
                        Ubicación
                    </button>
                    <button type="button" 
                            wire:click="setTab('capabilities')"
                            class="py-2 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'capabilities' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} transition-colors">
                        Capacidades
                    </button>
                    <button type="button" 
                            wire:click="setTab('contact')"
                            class="py-2 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'contact' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} transition-colors">
                        Contacto
                    </button>
                </nav>
            </div>
        ...
        {{-- Tab Content - Reutilizando el mismo contenido del Create pero con valores prellenados --}}
                ...
                <div class="text-sm text-gray-600">
                    @if($hasUnsavedChanges)
                        <div class="flex items-center text-yellow-700">
                            ...
                            <span class="font-medium">Tienes cambios sin guardar</span>
                        </div>
                    @else
                        <span class="font-medium">Todos los cambios guardados</span>
                    @endif
                </div>
                <div class="flex items-center space-x-3">
                    @if($hasUnsavedChanges)
                    ...
                    <button type="submit" 
                            class="px-6 py-2 text-sm font-medium text-white {{ $hasUnsavedChanges ? 'bg-yellow-600 hover:bg-yellow-700 focus:ring-yellow-500' : 'bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500' }} border border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors shadow-sm">
                        <svg wire:loading wire:target="save" class="inline w-4 h-4 mr-2 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                        </svg>
    ...
    <div x-show="showDiscardModal"
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 transform"
             x-transition:enter="transition ease-out duration-300"
            ...
            <div class="px-6 py-4">
                ...
                <h3 class="text-lg font-medium text-gray-900 text-center mb-2">Descartar Cambios</h3>
                <p class="text-sm text-gray-600 text-center mb-6">
                    Tienes cambios sin guardar. ¿Estás seguro que deseas salir sin guardar?
                </p>
                <div class="flex items-center justify-center space-x-3">
                    <button @click="showDiscardModal = false"
                            class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                        Continuar Editando
                    </button>
